...
def calcola_planning_fix(data_start, ora_start, piaz_h, pezzi_h):
    ...
    # Riferimento per l'alternanza turni (Lunedì della settimana di inizio)
    ...
    while (min_restanti_piaz + min_restanti_prod) > 0:
        ...
        if wd == 5: # Sabato
            ...
            if tipo_lavoro == "Due Turni (Continuo)":
                ...
                if "Mattina" in turno_sett:
                    fasce, pause = [(time(6, 0), time(13, 50))], [(time(12, 0), time(12, 20))]
                else:
                    fasce, pause = [(time(13, 50), time(21, 40))], [(time(19, 30), time(19, 50))]
        ...
        for f_ini, f_fin in fasce:
            ...
            while corrente < lim_fin and (min_restanti_piaz + min_restanti_prod) > 0:
                ...
                ora_inizio_dec = corrente.hour + corrente.minute/60.0
                
                if pausa_attiva:
                    durata_p = (pausa_attiva[1] - corrente).total_seconds() / 3600
                    log.append({"Giorno": corrente.strftime('%a %d/%m'), "Inizio": ora_inizio_dec, "Durata": durata_p, "Tipo": "PAUSA", "Label": f"{corrente.strftime('%H:%M')}-{pausa_attiva[1].strftime('%H:%M')}"})
                    corrente = pausa_attiva[1]
                    lavorato_oggi = True
                    continue

                # Calcolo lavoro
                ostacoli = [lim_fin] + [corrente.replace(hour=p[0].hour, minute=p[0].minute) for p in pause if corrente < corrente.replace(hour=p[0].hour, minute=p[0].minute)]
                ...
                min_da_fare = min(min_restanti_piaz if min_restanti_piaz > 0 else min_restanti_prod, min_disp)
                ...
                log.append({"Giorno": corrente.strftime('%a %d/%m'), "Inizio": ora_inizio_dec, "Durata": min_da_fare/60, "Tipo": tipo, "Label": f"{corrente.strftime('%H:%M')}-{fine_eff.strftime('%H:%M')}"})
                corrente = fine_eff
                lavorato_oggi = True
    ...
    # CORREZIONE ERRORE: data_fine diventa corrente
    return corrente, pd.DataFrame(log)

# --- RENDERING ---
if st.button("CALCOLA PLANNING"):
    dt_partenza = datetime.combine(data_inizio_val, ora_inizio_val)
    fine_lavoro, df = calcola_planning_fix(data_inizio_val, ora_inizio_val, piazzamento_ore, (n_pezzi*tempo_pezzo)/60)
    
    st.success(f"### 🏁 Fine Lavorazione stimata: {fine_lavoro.strftime('%A %d %B - ore %H:%M')}")

    if not df.empty:
        # Imponiamo l'ordine cronologico corretto dei giorni sulla X
        ...
        fig.update_layout(
            ...
            height=800,
            barmode='overlay' # Overlay è meglio per gestire il parametro 'base'
        )
